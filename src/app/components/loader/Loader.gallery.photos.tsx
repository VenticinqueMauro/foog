...
export default function LoaderGalleryPhotos() {
    ...
    return (
        <span className='flex flex-col h-screen gap-2 animate-pulse'>
            <span className="block h-6 mb-10 rounded w-52 bg-zinc-600">&nbsp;</span>
            <ul className="grid grid-cols-12 gap-2">
                {
                    skeletonImage.map((image, i) => <li key={i} className="block col-span-6 rounded md:col-span-4 xl:col-span-3 h-[300px] bg-zinc-600">&nbsp;</li>)
                }
            </ul>
        </span>
    )
